...
export async function loader({ params }: LoaderFunctionArgs) {
  ...
  try {
    ...
    const category = await api.getCategoryById(car.categoryId);
    const carWithCategoryName = {
      ...car,
      categoryName: category.title,
    };
    return carWithCategoryName;
  } catch (error) {
...
export default function CarDetailRoute({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-neutral-background">
      <Header />
      <Breadcrumbs
        items={[{ label: "Carros", path: "/" }, { label: car.title }]}
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center">
          ...
          <p className="text-body-md text-neutral-text mb-6">
            O carro que você procura não existe ou foi removido.
          </p>
          <a
            href="/"
