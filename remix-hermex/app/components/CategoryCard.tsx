...
export const CategoryCard = memo(function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute inset-0 bg-gradient-to-br from-primary-light/20 to-transparent group-hover:from-primary-pure/30 transition-colors"
        aria-hidden="true"
      />
      ...
      <article className="relative p-8">
        {/* Icon */}
        <div
          className="w-16 h-16 bg-primary-pure rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform"
          aria-hidden="true"
        >
        ...
        <div className="flex items-center justify-between">
          ...
          <div
            className="w-8 h-8 bg-primary-pure rounded-full flex items-center justify-center group-hover:translate-x-1 transition-transform"
            aria-hidden="true"
          >
      ...
      {/* Decorative corner */}
      <div
        className="absolute top-0 right-0 w-24 h-24 bg-secondary-pure/10 rounded-bl-full"
        aria-hidden="true"
      />
    </Link>
  );
